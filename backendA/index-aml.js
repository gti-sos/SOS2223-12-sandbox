        ...
        db.find({}, function(err,lista){
            ...
            if (lista === 0) {
                for(var i = 0; i<agroclimatic.length;i++){
                    db.insert(agroclimatic[i]);
                }
                response.sendStatus(200);
                console.log("Se han creado datos")
            } else {
                response.json('El arreglo ya contiene datos');
    ...
    */
    ...
    app.get(BASE_API_URL+"/agroclimatic", (request, response) => {
          ...
          // Lógica para buscar todas las ciudades en el período especificado
            ...
            const { year } = request.query;
                ...
                if (year) {
                    ...
                    return (s.year === parseInt(year));
                    });
                    console.log("Nuevo GET to /agroclimatic con año"); //console.log en el servidor  
                    response.status(200).json(lista.map((e)=>{
                    delete e._id;
                    return e;
                    }));
                }else {
              ...
              //if(err){
                ...
                //response.sendStatus(500);
              //}else{
                    console.log(`Agroclimatic devuelve ${lista.length}`);
                    response.status(200).json(lista.map((e)=>{
                    delete e._id;
                    return e;
                    }));
        ...
        db.find({}, (err, agroclimatic) =>{
            if(err){
                console.log("Error al buscar contactos");
                response.sendStatus(500);
            }else{
                console.log("Contactos devueltos");
    ...
    app.post(BASE_API_URL + "/agroclimatic", (request, response) => {
        ...
        db.insert(nuevo_dato);
        response.sendStatus(201);
        /*db.find({province : nuevo_dato.province})
    ...
    app.delete(BASE_API_URL+"/agroclimatic/:province", (request, response) => {
        ...
        db.remove({"province" : province}, {}, (err, numRemoved)=>{
            if(err){
                console.log("Error para borrar la provincia");
                response.sendStatus(500);
            }else{
                console.log("Se ha borrado la provincia");
                response.sendStatus(200);
            }
        });
        /*if (filtro.length === 0) {
